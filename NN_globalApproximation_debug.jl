...
for s = 1 : 1 : num_states
    ...
    for a = 1 : 1 : num_actions
        if Q[s,a] != 0
            ...
            X[i,3] = LIB
            X[i,4] = send_Hydro
            X[i,5] = send_LIBattery
            X[i,6] = pull_Hydro
            X[i,7] = pull_LIBattery
...
for s = 1 : 1 : num_states
    ...
    for a = 1 : 1 : num_actions
        if Q[s,a] != 0
            ...
            X_nv[i,3] = LIB
            X_nv[i,4] = send_Hydro
            X_nv[i,5] = send_LIBattery
            X_nv[i,6] = pull_Hydro
            X_nv[i,7] = pull_LIBattery
...
for s = 1 : 1 : num_states
    ...
    for a = 1 : 1 : num_actions
        ...
        X_all[i,2] = H
        X_all[i,3] = LIB
        X_all[i,4] = send_Hydro
        X_all[i,5] = send_LIBattery
        X_all[i,6] = pull_Hydro
        X_all[i,7] = pull_LIBattery
