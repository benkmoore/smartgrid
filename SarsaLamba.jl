...
for loop = 1 : 1 : 1000
    for t = 1 : num_rows
        ...
        #4) Find next state and observations at next state
        if t != num_rows
            ...
            #Get next action: t+1
            demand_prime = energy_Data[t+1,6]
            solar_prime = energy_Data[t+1,7]
            wind_prime = energy_Data[t+1,8]
            action_prime = get_action(solar_prime, wind_prime, demand_prime, Hydro_amount, Hydro_step, MaxRate_Hydro, LIBattery_amount,
                                        LIBattery_step, MaxRate_LIBattery, CAP_Hydro, CAP_LIBattery, actions_dict, η_pull_Hydro, η_pull_LIBattery, η_send_Hydro, η_send_LIBattery)
            alpha = 0.5
            Q[state_t,action_t] = Q[state_t,action_t] + (alpha*(r + Q[state_prime,action_prime] - Q[state_t,action_t]))
        end
    end
end
#print(Q)
using JLD;
save("C:\\Users\\Ben\\Documents\\smartgridrl\\Q_SL.jld", "data", Q)
